...
export default function WorkOrderForm({ initial = {}, machines = [], onSuccess, onCancel }) {
  // coerce null -> {} because caller may pass `null` explicitly (default param only applies for undefined)
  const safeInitial = initial || {};

  const [form, setForm] = useState(() => {
    const si = safeInitial;
    const scheduled_date = si.scheduled_date
      ? new Date(si.scheduled_date).toISOString().split('T')[0]
      : '';
    return {
      order_number: '',
      machine_id: '',
  ...
  // keep form in sync when initial (possibly null) changes
  useEffect(() => {
    const si = initial || {};
    if (Object.keys(si).length > 0) {
      const formatted = {
        ...si,
        scheduled_date: si.scheduled_date ? new Date(si.scheduled_date).toISOString().split('T')[0] : ''
      };
      setForm(prev => ({ ...prev, ...formatted }));
    } else {
      // reset to defaults when there's no initial data
      setForm({
        order_number: '',
        machine_id: '',
        component_id: '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // determine edit vs create safely
            ...
            >
              ...
              {machines.map(m => (
                <option key={m._id ?? m.id} value={m._id ?? m.id}>
                  {m.name ?? m.model ?? m.serial_number}
                </option>
              ))}
            </select>
          </div>
